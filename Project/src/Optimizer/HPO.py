...
from src.model_runner import evaluate_model
...
def HPO_runner(
    ...
    def HPO(SearchAgents, Max_iterations, param_ranges, model, X_train, y_train, X_test, y_test):
        ...
        # Use randint for integer positions
        Positions = np.array([
            [np.random.randint(low=lowerbound[j], high=upperbound[j]+1) for j in range(dimension)]
            for _ in range(SearchAgents)
        ])
        ...
        for t in range(Max_iterations):
            ...
            Positions = Positions + np.round(0.01 * steps * (Positions - best_pos)).astype(int)
    ...
    if task_type == "regression":
        ...
        return best_params, best_fit, np.array(convergence_curve)
