...
def train_raw_model(model_name, X_train, y_train, X_test, y_test, params=None):
    ...
    # Split test set into halves
    midpoint = len(y_test) // 2
    y_test_1, y_test_2 = y_test[:midpoint], y_test[midpoint:]
    y_pred_test_1, y_pred_test_2 = y_pred_test[:midpoint], y_pred_test[midpoint:]

    # Evaluate
    metrics_train = evaluate_model(y_train, y_pred_train)
    metrics_test = evaluate_model(y_test, y_pred_test)
    metrics_test_1 = evaluate_model(y_test_1, y_pred_test_1)
    metrics_test_2 = evaluate_model(y_test_2, y_pred_test_2)

    # Bundle all metrics
    all_metrics = {
        "train": metrics_train,
        "test": metrics_test,
        "value": metrics_test_1,
        "test_value": metrics_test_2,
    }

    return {
        "model_name": model_name,
        # if there is no params, dont return model
        "model": model if params is not None else None,
